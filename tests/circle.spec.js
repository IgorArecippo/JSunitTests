...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    let circulo = circle('8');
    expect(circulo).toBeUndefined();
    expect(typeof circle(3)).toEqual('object');
    circulo = circle(2);
    expect(Object.keys(circulo).length).toEqual(3);
    expect(circle()).toBeUndefined();
    expect(Object.values(circulo)[2]).toEqual(12.56);
    circulo = circle(3);
    expect(Object.values(circulo)[1]).toBeCloseTo(28.259);
